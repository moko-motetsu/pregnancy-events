...
- **設備アイコン表示** — `tags`に含まれる「ベビーカーOK」「屋内」「屋外」「全天候型」「授乳室あり」「雨天決行」等の既知キーワードをアイコン付きバッジとしてカード・モーダルに表示（`FACILITY_TAG_ICONS`）。
- **開催状況バッジ** — 開催日から「開催中」「まもなく開催（あとN日）」「終了」を自動判定して表示（`eventStatus()`）。
- **クイック日付フィルタ** — 「今日」「明日」「今週末」のワンタップ絞り込みチップ。
- **地図表示** — Leaflet + OpenStreetMapで、絞り込み後のイベントを地図上にピン表示（リスト表示とワンタップで切り替え可能）。
- **天気予報連携** — 開催日が16日以内のイベントは、詳細モーダルにOpen-Meteo（APIキー不要）の天気予報を表示。
- **家族へのシェア** — イベント詳細から共有ボタンでLINE等に送信可能（`#event=イベントID`のディープリンク付き。共有されたリンクを開くと自動的に該当イベントのモーダルが開く）。
...
### 自治体・地域情報サイトのRSSフィードによるイベント自動取得（`rss_events.json`）

`pregnancy-events/scripts/fetch_rss_events.mjs` の `FEEDS` 定数に登録したRSS/AtomフィードからイベントRSSを取得し、`rss_events.json` に保存します。`events.json`（手動メンテナンス分）とは別ファイルで管理し、アプリ側の `mergeRssEvents()` で両方をマージ表示します（idが重複する場合は手動データを優先）。このファイルも自動生成物なので手動編集しないでください。

- 実行方法: GitHub Actions の「RSSイベント取得」ワークフロー（`.github/workflows/watch_rss_events.yml`）。毎日6:45 JSTに自動実行。
- 対応フィード:
  - 東京ミッドタウン（`tokyo-midtown`）: 単一施設のイベントフィード。開催場所・座標は固定値、開催日は個別ページの `【開催日時】` 表記から取得。
  - とよすと（`toyosu-tokyo`、豊洲エリアの地域情報サイト）: フィードが単一施設ではなく豊洲・有明・お台場エリア全体を扱うため、開催場所は記事ごとに `og:description` の文中（例:「…お台場海浜公園にて…」）から抽出する。座標は過去に確認済みの主要施設（有明ガーデン／有明ガーデンシアター／有明アリーナ／有明GYM-EX／お台場海浜公園／豊洲公園）のみ対応し、それ以外の会場は座標なしで掲載される（地図・距離表示は出ないが一覧・検索には表示される）。
- 新しいフィードを追加する場合は `FEEDS` に1件追記し、`detailParser`（個別ページから開催日等を抽出する関数）・`mapCategory`（カテゴリ分類）・`buildVenue`（会場名の組み立て）を用意する。フィードが単一施設のものであれば東京ミッドタウンの実装を、地域全体を扱うものであればとよすとの実装を参考にすること。

...
- **花火大会の開催日自動確認** — `events`に載せた花火大会について、`pregnancy-events/scripts/fetch_hanabi_events.mjs`が各イベントの`officialUrl`（自治体・観光協会・実行委員会など一次情報源のみ。まとめサイトは利用規約上の懸念があるため対象外）を確認し、`hanabi_events.json`に確認結果を保存する（GitHub Actions `watch_hanabi_events.yml`）。荒天順延などで日程が変わった場合、確認が取れ次第自動で反映される。
...
### 花火大会の開催日自動確認（`hanabi_events.json`）

`events` 内でカテゴリが「祭り・花火」かつ `officialUrl` を持つイベントについて、`pregnancy-events/scripts/fetch_hanabi_events.mjs` が `officialUrl` のページからJSON-LDで開催日を確認し、`hanabi_events.json` に保存します。`events.json` 自体は書き換えず、確認が取れたイベントだけアプリ側で表示時に上書きする仕組みです。このファイルも自動生成物なので手動編集しないでください。

- 実行方法: GitHub Actions の「花火大会 開催日確認」ワークフロー（`.github/workflows/watch_hanabi_events.yml`）。実サイトでの動作検証が済むまでは `workflow_dispatch` のみが有効。
- **対象は自治体・観光協会・実行委員会などの一次情報源のみ**。ウォーカープラス等のまとめサイトは利用規約で無断転載を禁止しているため、対象に含めないこと。新しい花火大会を追加する際は、必ず主催者・自治体の公式サイトを `officialUrl` に設定する。
